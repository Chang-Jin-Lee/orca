 ...
 * RPC. Success proves the whole headless path works under plain Node + Linux:
...
const SERVER_ENTRY = process.argv[2] || '/app/orca-server.js'
const fail = (msg) => {
...
const child = spawn('node', [SERVER_ENTRY, '--serve-port', '0', '--json'], {
...
async function runClient(ready) {
  ...
  const callTimeout = setTimeout(() => fail(`handshake/RPC stalled in phase=${phase} within 15s`), 15_000)
  ...
  ws.on('message', (data) => {
    ...
    if (phase === 'hello' && msg.type === 'e2ee_ready') {
      ...
      ws.send(encrypt(JSON.stringify({ type: 'e2ee_auth', deviceToken: pairing.deviceToken }), sharedKey))
      return
    }
    ...
    if (phase === 'rpc' && msg.id === RPC_ID) {
      ...
      if (!msg.result || msg.result.platform !== 'linux') {
        return fail(`expected platform=linux, got ${JSON.stringify(msg.result)}`)
      }
      console.log(
        'SMOKE PASS: headless server booted, paired, E2EE handshake + encrypted RPC succeeded on Linux/Node with no Electron.'
      )
      ws.close()
      child.kill('SIGTERM')
      process.exit(0)
