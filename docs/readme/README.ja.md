...
Linux では CLI は `orca-ide` としてインストールされます（`orca` という名前は GNOME Orca スクリーンリーダーが使用しているためです）。`orca` の代わりに `orca-ide` を使うか、そのスクリーンリーダーを使っていない場合は短い名前のシンボリックリンクを作成してください: `mkdir -p ~/.local/bin && ln -s "$(command -v orca-ide)" ~/.local/bin/orca`。
