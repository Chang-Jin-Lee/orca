...
在 Linux 上，该 CLI 安装为 `orca-ide`（`orca` 这个名称已被 GNOME Orca 屏幕阅读器占用）。请用 `orca-ide` 代替 `orca`；如果你不使用该屏幕阅读器，也可以为短名称创建符号链接：`mkdir -p ~/.local/bin && ln -s "$(command -v orca-ide)" ~/.local/bin/orca`。
