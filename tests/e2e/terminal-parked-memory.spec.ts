...
// mutation path as the kill-switch test below.
async function enableTerminalParking(page: Page): Promise<void> {
  await page.evaluate(async () => {
    const store = window.__store
    if (!store) {
      throw new Error('parking spec: window.__store is unavailable')
    }
    await store.getState().updateSettings({ terminalHiddenViewParking: true })
  })
}
